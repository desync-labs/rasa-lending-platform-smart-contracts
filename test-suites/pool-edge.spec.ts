...
import { deployMintableERC20 } from '../helpers/contract-deployments';
...
// Setup function to have 1 user with RUSD deposits, and another user with WETH collateral
// and RUSD borrowings at an indicated borrowing mode
const setupPositions = async (testEnv: TestEnv, borrowingMode: RateMode) => {
  ...
  const amountRUSDtoDeposit = await convertToCurrencyDecimals(rusd.address, '10000');
  ...
  const amountRUSDToBorrow = await convertToCurrencyDecimals(
    rusd.address,
    userGlobalData.availableBorrowsBase.div(rusdPrice).mul(5000).div(10000).toString()
  );
  await pool
    .connect(borrower.signer)
    .borrow(rusd.address, amountRUSDToBorrow, borrowingMode, '0', borrower.address);
};
...
makeSuite('Pool: Edge cases', (testEnv: TestEnv) => {
  ...
  it('Drop asset while user uses it as collateral, ensure that borrowing power is lowered', async () => {
    ...
    await rusd.connect(user0.signer).approve(mockPool.address, MAX_UINT_AMOUNT);

    expect(await mockPool.connect(user0.signer).supply(rusd.address, amount, user0.address, 0));

    const userReserveDataBefore = await mockPool.getUserAccountData(user0.address);

    expect(userReserveDataBefore.totalCollateralBase).to.be.eq(amount);
    expect(userReserveDataBefore.totalDebtBase).to.be.eq(0);
    expect(userReserveDataBefore.availableBorrowsBase).to.be.eq(amount.mul(7500).div(10000));
    ...
    expect(await mockPool.dropReserve(rusd.address));
  ...
  it('Tries to initialize a reserve as non PoolConfigurator (revert expected)', async () => {
    const { pool, users, rusd, helpersContract } = testEnv;

    const config = await helpersContract.getReserveTokensAddresses(rusd.address);
  ...
  it('Call `setUserUseReserveAsCollateral()` to use an asset as collateral when the asset is already set as collateral', async () => {
    ...
    expect(await pool.connect(user0.signer).supply(rusd.address, amount, user0.address, 0));
    ...
    expect(
      await pool.connect(user0.signer).setUserUseReserveAsCollateral(rusd.address, true)
  ...
  it('Call `setUserUseReserveAsCollateral()` to disable an asset as collateral when the asset is already disabled as collateral', async () => {
    ...
    await expect(pool.connect(user0.signer).setUserUseReserveAsCollateral(rusd.address, false))
    ...
    expect(
      await pool.connect(user0.signer).setUserUseReserveAsCollateral(rusd.address, false)
  ...
  it('Call `mintToTreasury()` on a pool with an inactive reserve', async () => {
    const { pool, poolAdmin, rusd, users, configurator } = testEnv;

    // Deactivate reserve
    expect(await configurator.connect(poolAdmin.signer).setReserveActive(rusd.address, false));

    // MintToTreasury
    expect(await pool.connect(users[0].signer).mintToTreasury([rusd.address]));
  });

  it('Tries to call `finalizeTransfer()` by a non-RSToken address (revert expected)', async () => {
    const { pool, rusd, users } = testEnv;

    await expect(
      pool
        .connect(users[0].signer)
        .finalizeTransfer(rusd.address, users[0].address, users[1].address, 0, 0, 0)
    ).to.be.revertedWith(CALLER_NOT_RSTOKEN);
  });
  ...
  it('PoolConfigurator updates the ReserveInterestRateStrategy address', async () => {
    const { pool, deployer, rusd, configurator } = testEnv;
    ...
    expect(
      await pool
        .connect(configSigner)
        .setReserveInterestRateStrategyAddress(rusd.address, ZERO_ADDRESS)
    );

    const config = await pool.getReserveData(rusd.address);
  ...
  it('PoolConfigurator updates the ReserveInterestRateStrategy address for an unlisted asset (revert expected)', async () => {
    const { pool, deployer, rusd, configurator, users } = testEnv;
  ...
  it('Initialize an already initialized reserve. ReserveLogic `init` where RSTokenAddress != ZERO_ADDRESS (revert expected)', async () => {
    const { pool, rusd, deployer, configurator } = testEnv;
    ...
    await expect(
      pool.connect(configSigner).initReserve(
        rusd.address,
        config.RSTokenAddress, // just need a non-used reserve token
        config.stableDebtTokenAddress,
        config.variableDebtTokenAddress,
        ZERO_ADDRESS
      )
  ...
  it('Init reserve with ZERO_ADDRESS as RSToken twice, to enter `_addReserveToList()` already added (revert expected)', async () => {
    /**
     * To get into this case, we need to init a reserve with `RSTokenAddress = address(0)` twice.
     * `_addReserveToList()` is called from `initReserve`. However, in `initReserve` we run `init` before the `_addReserveToList()`,
     * and in `init` we are checking if `RSTokenAddress == address(0)`, so to bypass that we need this odd init.
     */
    const { pool, rusd, deployer, configurator } = testEnv;
  ...
  it('Initialize reserves until max, then add one more (revert expected)', async () => {
    ...
    const { addressesProvider, poolAdmin, pool, rusd, deployer, configurator } = testEnv;
    const { deployer: deployerName } = await hre.getNamedAccounts();
  ...
  it('Initialize reserves until max-1, then (drop one and add a new) x 2, finally add to hit max', async () => {
    ...
    for (let dropped = 0; dropped < 2; dropped++) {
      ...
      for (let i = 0; i < reservesListBefore.length; i++) {
        ...
        if (assetData.RSTokenAddress == ZERO_ADDRESS) {
          continue;
        }
  ...
  it('Call `resetIsolationModeTotalDebt()` to reset isolationModeTotalDebt of an asset with non-zero debt ceiling', async () => {
    ...
    expect(await helpersContract.getDebtCeiling(rusd.address)).to.be.eq(0);

    await configurator.connect(poolAdmin.signer).setDebtCeiling(rusd.address, debtCeiling);

    expect(await helpersContract.getDebtCeiling(rusd.address)).to.be.eq(debtCeiling);
    ...
    await expect(
      pool.connect(configSigner).resetIsolationModeTotalDebt(rusd.address)
    ).to.be.revertedWith(DEBT_CEILING_NOT_ZERO);
  });

  it('Tries to initialize a reserve with an RSToken, StableDebtToken, and VariableDebt each deployed with the wrong pool address (revert expected)', async () => {
    ...
    const RSTokenImp = await new RSToken__factory(await getFirstSigner()).deploy(pool.address);
    ...
    const RSTokenWrongPool = await new RSToken__factory(await getFirstSigner()).deploy(
    ...
    const initInputParams: {
      ...
      {
        RSTokenImpl: RSTokenImp.address,
    ...
    initInputParams[0].RSTokenImpl = RSTokenWrongPool.address;
    await expect(configurator.initReserves(initInputParams)).to.be.reverted;

    initInputParams[0].RSTokenImpl = RSTokenImp.address;
  ...
  it('dropReserve(). Only allows to drop a reserve if both the RSToken supply and accruedToTreasury are 0', async () => {
    const {
      configurator,
      pool,
      weth,
    ...
    const collectorAddress = await RSWETH.RESERVE_TREASURY_ADDRESS();
  ...
  it('validateSupply(). Only allows to supply if amount + (scaled RSToken supply + accruedToTreasury) <= supplyCap', async () => {
    const {
      configurator,
      pool,
      weth,
  ...
  it('_checkNoSuppliers() (PoolConfigurator). Properly disables actions if RSToken supply == 0, but accruedToTreasury != 0', async () => {
    const {
      configurator,
      pool,
      weth,
  ...
  it('LendingPool Reserve Factor 100%. Only variable borrowings. Validates that variable borrow index accrue, liquidity index not, and the Collector receives accruedToTreasury allocation after interest accrues', async () => {
    ...
    // Set the RF to 100%
    await configurator.setReserveFactor(rusd.address, '10000');

    const reserveDataBefore = await pool.getReserveData(rusd.address);
    ...
    await pool
      ...
      .deposit(
        rusd.address,
        await convertToCurrencyDecimals(rusd.address, '1'),
        depositor.address,
        '0'
      );

    const reserveDataAfter1 = await pool.getReserveData(rusd.address);
    ...
    const reserveDataAfter2 = await pool.getReserveData(rusd.address);
  ...
  it('Pool with non-zero unbacked keeps the same liquidity and debt rate, even while setting zero unbackedMintCap', async () => {
    ...
    // User 3 supplies 1M RUSD and borrows 0.25M RUSD
    const rusdAmount = await convertToCurrencyDecimals(rusd.address, '1000000');
    expect(await rusd.connect(user3.signer)['mint(uint256)'](rusdAmount));
    expect(await rusd.connect(user3.signer).approve(pool.address, MAX_UINT_AMOUNT));
    expect(await pool.connect(user3.signer).deposit(rusd.address, rusdAmount, user3.address, '0'));
    expect(
      await pool
        .connect(user3.signer)
        .borrow(rusd.address, rusdAmount.div(4), RateMode.Variable, '0', user3.address)
    );
    ...
    // Set non-zero unbackedMintCap for RUSD
    expect(await configurator.setUnbackedMintCap(rusd.address, MAX_UNBACKED_MINT_CAP));

    // Bridge mints 1M unbacked aRUSD on behalf of User 1
    expect(
      await pool.connect(bridge.signer).mintUnbacked(rusd.address, rusdAmount, user1.address, 0)
    );

    const reserveDataBefore = await getReserveData(helpersContract, rusd.address);

    expect(await rusd.connect(user2.signer)['mint(uint256)'](rusdAmount));
    expect(await rusd.connect(user2.signer).approve(pool.address, MAX_UINT_AMOUNT));

    // Next two txs should be mined in the same block
    await setAutomine(false);

    // Set zero unbackedMintCap for RUSD
    expect(await configurator.setUnbackedMintCap(rusd.address, 0));

    // User 2 supplies 10 RUSD
    const amountToDeposit = await convertToCurrencyDecimals(rusd.address, '10');
    const tx = await pool
      .connect(user2.signer)
      .deposit(rusd.address, amountToDeposit, user2.address, '0');

...
makeSuite('Upgradeability', (testEnv: TestEnv) => {
  ...
  context('PoolConfigurator upgrade ability', () => {
    ...
    let newRSTokenAddress: string;
    let newStableTokenAddress: string;
    let newVariableTokenAddress: string;

    before('deploying instances', async () => {
      const { rusd, pool } = testEnv;
      const RSTokenInstance = await deployMockRSToken([
        pool.address,
        rusd.address,
        ZERO_ADDRESS,
        ZERO_ADDRESS,
        'RASA Interest bearing RUSD updated',
        'aRUSD',
        '0x10',
      ]);
      ...
      const stableDebtTokenInstance = await deployMockStableDebtToken([
        ...
        rusd.address,
        ZERO_ADDRESS,
        'RASA stable debt bearing RUSD updated',
        'stableDebtRUSD',
        '0x10',
      ]);
      ...
      const variableDebtTokenInstance = await deployMockVariableDebtToken([
        ...
        rusd.address,
        ZERO_ADDRESS,
        'RASA variable debt bearing RUSD updated',
        'variableDebtRUSD',
        '0x10',
      ]);

      newRSTokenAddress = RSTokenInstance.address;
      newVariableTokenAddress = variableDebtTokenInstance.address;
      newStableTokenAddress = stableDebtTokenInstance.address;
    });

    it('Tries to update the RUSD RSToken implementation with a different address than the poolManager', async () => {
      const { rusd, configurator, users } = testEnv;

      const name = await (await getRSToken(newRSTokenAddress)).name();
      const symbol = await (await getRSToken(newRSTokenAddress)).symbol();

      const updateRSTokenInputParams: {
        asset: string;
        treasury: string;
        ...
        name: name,
        symbol: symbol,
        implementation: newRSTokenAddress,
        params: '0x10',
      };
      await expect(
        configurator.connect(users[1].signer).updateRSToken(updateRSTokenInputParams)
      ).to.be.revertedWith(CALLER_NOT_POOL_ADMIN);
    });

    it('Upgrades the RUSD RSToken implementation ', async () => {
      const { rusd, configurator, RSRUSD } = testEnv;
      ...
      await configurator.updateRSToken(updateRSTokenInputParams);

      const tokenName = await RSRUSD.name();

      expect(tokenName).to.be.eq('RASA Interest bearing RUSD updated', 'Invalid token name');
    });

    it('Tries to update the RUSD Stable debt token implementation with a different address than the poolManager', async () => {
      const { rusd, configurator, users } = testEnv;
    ...
    it('Upgrades the RUSD stable debt token implementation ', async () => {
      const { rusd, configurator, helpersContract } = testEnv;
      ...
      expect(tokenName).to.be.eq('RASA stable debt bearing RUSD updated', 'Invalid token name');
    });

    it('Tries to update the RUSD variable debt token implementation with a different address than the poolManager', async () => {
      const { rusd, configurator, users } = testEnv;
    ...
    it('Upgrades the RUSD variable debt token implementation ', async () => {
      const { rusd, configurator, helpersContract } = testEnv;
      ...
      expect(tokenName).to.be.eq('RASA variable debt bearing RUSD updated', 'Invalid token name');
    });
  });
});

...
import { getStableDebtToken } from '../helpers/contract-getters';
...
makeSuite('StableDebtToken', (testEnv: TestEnv) => {
  ...
  it('Check initialization', async () => {
    ...
    const rusdStableDebtTokenAddress = (await helpersContract.getReserveTokensAddresses(rusd.address))
      .stableDebtTokenAddress;
    const stableDebtContract = StableDebtToken__factory.connect(
      rusdStableDebtTokenAddress,
      users[0].signer
    );

    expect(await stableDebtContract.UNDERLYING_ASSET_ADDRESS()).to.be.eq(rusd.address);
  ...
  it('Tries to burn not being the Pool (revert expected)', async () => {
    ...
    expect(name).to.be.equal('RASA Stable Debt RUSD');
  ...
  it('Check Mint and Transfer events when borrowing on behalf', async () => {
    ...
    // Add EURS liquidity
    await eurs.connect(user3.signer)['mint(uint256)'](utils.parseUnits('1000', 2));
    await eurs.connect(user3.signer).approve(pool.address, MAX_UINT_AMOUNT);
    await pool
      .connect(user3.signer)
      .supply(eurs.address, utils.parseUnits('1000', 2), user3.address, 0);
    ...
    const stableDebtToken = StableDebtToken__factory.connect(
      eursData.stableDebtTokenAddress,
      user1.signer
    );
    const beforeDebtBalanceUser2 = await stableDebtToken.balanceOf(user2.address);

    // User1 borrows 100 EURS
    const borrowAmount = utils.parseUnits('100', 2);
    expect(
      await pool
        .connect(user1.signer)
        .borrow(eurs.address, borrowAmount, RateMode.Stable, 0, user1.address)
    );

    // User1 approves user2 to borrow 1000 EURS
    expect(
      await stableDebtToken
        .connect(user1.signer)
        .approveDelegation(user2.address, utils.parseUnits('1000', 2))
    );

    // Increase time so interests accrue
    await increaseTime(24 * 3600);

    // User2 borrows 1000 EURS on behalf of user1
    const borrowOnBehalfAmount = utils.parseUnits('100', 2);
    const tx = await waitForTx(
      await pool
        .connect(user2.signer)
        .borrow(eurs.address, borrowOnBehalfAmount, RateMode.Stable, 0, user1.address)
    );
  ...
  it('User borrows and repays in same block with zero fees', async () => {
    const { pool, users, rusd, RSRUSD, eurs, stableDebtRUSD } = testEnv;
    const user = users[0];
    ...
    expect(await stableDebtRUSD.balanceOf(user.address)).to.be.eq(0);
    ...
    expect(
      await pool
        .connect(user.signer)
        .repay(rusd.address, utils.parseEther('500'), RateMode.Stable, user.address)
    );
  ...
  it('User borrows and repays in same block using credit delegation with zero fees', async () => {
    ...
    const stableDebtToken = await getStableDebtToken(rusdData.stableDebtTokenAddress);
    ...
    expect(
      await pool
        .connect(user2.signer)
        .borrow(rusd.address, utils.parseEther('2'), RateMode.Stable, 0, user1.address)
    );
    ...
    expect(
      await pool
        .connect(user1.signer)
        .repay(rusd.address, utils.parseEther('2'), RateMode.Stable, user1.address)
    );

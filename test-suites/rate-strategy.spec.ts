...
import { RSToken, DefaultReserveInterestRateStrategy, MintableERC20 } from '../types';
import { strategyRUSD } from '../markets/test/reservesConfigs';
import { rateStrategyStableTwo } from '../markets/test/rateStrategies';
...
makeSuite('InterestRateStrategy', (testEnv: TestEnv) => {
  ...
  it('Checks rates at 0% usage ratio, empty reserve', async () => {
    let params: CalculateInterestRatesParams = {
      ...
      reserveFactor: strategyRUSD.reserveFactor,
  ...
  it('Checks rates at 80% usage ratio', async () => {
    ...
    expect(currentLiquidityRate).to.be.equal(
      expectedVariableRate
        .percentMul(8000)
        .percentMul(BigNumber.from(PERCENTAGE_FACTOR).sub(strategyRUSD.reserveFactor)),
  ...
  it('Checks rates at 100% usage ratio', async () => {
    ...
    expect(currentLiquidityRate).to.be.equal(
      expectedVariableRate.percentMul(
        BigNumber.from(PERCENTAGE_FACTOR).sub(strategyRUSD.reserveFactor)
      ),
      'Invalid liquidity rate'
    );

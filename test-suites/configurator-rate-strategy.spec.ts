...
makeSuite('PoolConfigurator: Set Rate Strategy', (testEnv: TestEnv) => {
  ...
  it('Update Interest Rate of a reserve', async () => {
    ...
    // Utilize the RUSD pool
    const mintedAmount = utils.parseEther('100');
    expect(await rusd.connect(depositor.signer)['mint(uint256)'](mintedAmount));
    expect(await rusd.connect(depositor.signer).approve(pool.address, MAX_UINT_AMOUNT));
    expect(
      await pool.connect(depositor.signer).deposit(rusd.address, mintedAmount, depositor.address, 0)
    );
    ...
    expect(
      await pool
        ...
        .borrow(rusd.address, utils.parseEther('1'), 1, 0, borrower.address)
    );

    // PoolAdmin updates IR strategy address
    const strategyEURS = await helpersContract.getInterestRateStrategyAddress(eurs.address);
    const reserveDataBefore = await pool.getReserveData(rusd.address);

    await expect(
      configurator
        .connect(poolAdmin.signer)
        .setReserveInterestRateStrategyAddress(rusd.address, strategyEURS)
    )
      .to.emit(configurator, 'ReserveInterestRateStrategyChanged')
      .withArgs(rusd.address, reserveDataBefore.interestRateStrategyAddress, strategyEURS);

    const reserveDataAfter = await pool.getReserveData(rusd.address);

    expect(reserveDataAfter.interestRateStrategyAddress).to.be.eq(strategyEURS);
    ...
    // Rates get updated
    const reserveDataUpdated = await pool.getReserveData(rusd.address);
  ...
  it('Update Interest Rate of a reserve with ZERO_ADDRESS and bricks the reserve (revert expected)', async () => {
    ...
    await expect(
      ...
      .to.emit(configurator, 'ReserveInterestRateStrategyChanged')
      .withArgs(rusd.address, reserveDataBefore.interestRateStrategyAddress, ZERO_ADDRESS);
  ...
  it('ZeroReserveInterestRateStrategy - Update a reserve with ZeroInterestRateStrategy to set zero rates', async () => {
    ...
    expect(
      await pool
        .connect(borrower.signer)
        .borrow(rusd.address, utils.parseEther('1'), 2, 0, borrower.address)
    );
    ...
    expect(
      await pool
        .connect(stableBorrower.signer)
        .borrow(rusd.address, utils.parseEther('1'), 1, 0, stableBorrower.address)
    );
    ...
    await expect(
      configurator
        .connect(poolAdmin.signer)
        .setReserveInterestRateStrategyAddress(rusd.address, zeroStrategy.address)
    )
      .to.emit(configurator, 'ReserveInterestRateStrategyChanged')
      .withArgs(rusd.address, reserveDataBefore.interestRateStrategyAddress, zeroStrategy.address);
    ...
    await expect(
      pool.connect(depositor.signer).rebalanceStableBorrowRate(rusd.address, stableBorrower.address)
    )
      .to.emit(pool, 'RebalanceStableBorrowRate')
      .withArgs(rusd.address, stableBorrower.address);

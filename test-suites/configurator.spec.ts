...
import { strategyWETH } from '../markets/test/reservesConfigs';
...
const getReserveData = async (helpersContract: ProtocolDataProvider, asset: string) => {
  return Promise.all([
...
makeSuite('PoolConfigurator', (testEnv: TestEnv) => {
  ...
  it('Set a eMode category to an asset', async () => {
    const { configurator, pool, helpersContract, poolAdmin, rusd } = testEnv;

    const oldCategoryId = await helpersContract.getReserveEModeCategory(rusd.address);

    const newCategoryId = 1;

    expect(await configurator.connect(poolAdmin.signer).setAssetEModeCategory(rusd.address, '1'))
      .to.emit(configurator, 'EModeAssetCategoryChanged')
      .withArgs(rusd.address, oldCategoryId, newCategoryId);
  ...
  it('Resets the siloed borrowing mode. Tries to set siloed borrowing after the asset has been borrowed (revert expected)', async () => {
    ...
    const rusdAmount = utils.parseEther('1000');
    ...
    // user 2 supplies RUSD, borrows WETH
    await rusd.connect(user2.signer)['mint(uint256)'](rusdAmount);

    await rusd.connect(user2.signer).approve(pool.address, MAX_UINT_AMOUNT);

    await pool.connect(user2.signer).supply(rusd.address, rusdAmount, user2.address, '0');
  ...
  it('Check that the reserves have flashloans enabled', async () => {
    const { weth, cgo, eurs, rusd, helpersContract } = testEnv;
    ...
    const cgoFlashLoanEnabled = await helpersContract.getFlashLoanEnabled(cgo.address);
    expect(cgoFlashLoanEnabled).to.be.equal(true);

    const eursFlashLoanEnabled = await helpersContract.getFlashLoanEnabled(eurs.address);
    expect(eursFlashLoanEnabled).to.be.equal(true);

    const rusdFlashLoanEnabled = await helpersContract.getFlashLoanEnabled(rusd.address);
    expect(rusdFlashLoanEnabled).to.be.equal(true);
  });

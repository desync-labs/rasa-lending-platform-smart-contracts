...
import { waitForTx, advanceTimeAndBlock } from '../helpers/utilities/tx';
...
makeSuite('BridgeLogic: Testing with borrows', (testEnv: TestEnv) => {
  ...
  const borrowAmount = utils.parseEther('20');
  ...
  it('User 0 deposit 1000 rusd.', async () => {
    const { users, pool, rusd } = testEnv;
    await waitForTx(await rusd.connect(users[0].signer)['mint(uint256)'](depositAmount));
    await waitForTx(await rusd.connect(users[0].signer).approve(pool.address, MAX_UINT_AMOUNT));
    await waitForTx(
      await pool.connect(users[0].signer).deposit(rusd.address, depositAmount, users[0].address, 0)
    );
  });

  it('User 1 deposit 1000 ETH', async () => {
    const { users, pool, weth } = testEnv;
    await waitForTx(await weth.connect(users[1].signer).deposit({ value: utils.parseEther('1000') }));
    await waitForTx(await weth.connect(users[1].signer).approve(pool.address, MAX_UINT_AMOUNT));
    await waitForTx(
      await pool
        .connect(users[1].signer)
        .deposit(weth.address, utils.parseEther('1000'), users[1].address, 0)
    );
  });

  it('User 1 borrows 20 rusd with variable debt', async () => {
    const { users, pool, rusd } = testEnv;
    await waitForTx(
      await pool
        .connect(users[1].signer)
        .borrow(rusd.address, borrowAmount, RateMode.Variable, 0, users[1].address)
    );
  });

  it('User 1 borrows 20 rusd with stable debt', async () => {
    const { users, pool, rusd } = testEnv;
    await waitForTx(
      await pool
        .connect(users[1].signer)
        .borrow(rusd.address, borrowAmount, RateMode.Stable, 0, users[1].address)
    );
  });

  it('User 1 tries to perform fast withdraw 100 RSRUSD from L2 (revert expected)', async () => {
    const { users, pool, rusd } = testEnv;
    await expect(
      pool.connect(users[1].signer).mintUnbacked(rusd.address, mintAmount, users[0].address, 0)
  ...
  it('User 2 tries to perform fast withdraw from L2 with no unbackedMintCap (revert expected)', async () => {
    const { users, pool, rusd } = testEnv;
    // fast withdraw a100 RUSD
    await expect(
      pool.connect(users[2].signer).mintUnbacked(rusd.address, mintAmount, users[0].address, 0)
    ).to.be.revertedWith(UNBACKED_MINT_CAP_EXCEEDED);

    // fast withdraw 0 aRUSD
    await expect(
      pool.connect(users[2].signer).mintUnbacked(rusd.address, 0, users[0].address, 0)
    ).to.be.revertedWith(INVALID_AMOUNT);
  });

  it('RiskAdmin updates the unbackedMintCap to 10 RSRUSD (10 left) and user 1 tries to perform fast withdraw 100 RSRUSD from L2 (revert expected)', async () => {
    const { users, riskAdmin, pool, configurator, rusd } = testEnv;
  ...
  it('User 2 perform fast withdraw 100 RSRUSD from L2', async () => {
    ...
    const tx = await waitForTx(
      await pool.connect(users[2].signer).mintUnbacked(rusd.address, mintAmount, users[2].address, 0)
    );
  ...
  it('RiskAdmin updates the unbackedMintCap to 100 RSRUSD (0 left) and user 1 tries to perform fast withdraw 1 RSRUSD from L2 (revert expected)', async () => {
    const { users, riskAdmin, pool, configurator, rusd } = testEnv;
    expect(await configurator.connect(riskAdmin.signer).setUnbackedMintCap(rusd.address, '100'));
    await expect(
      pool.connect(users[2].signer).mintUnbacked(rusd.address, mintAmount, users[0].address, 0)
    ).to.be.revertedWith(UNBACKED_MINT_CAP_EXCEEDED);
  ...
  it('User 2 perform another fast withdraw 100 RSRUSD from L2', async () => {
    const { users, pool, rusd, helpersContract } = testEnv;
    const reserveDataBefore = await getReserveData(helpersContract, rusd.address);
    const tx = await waitForTx(
      await pool.connect(users[2].signer).mintUnbacked(rusd.address, mintAmount, users[2].address, 0)
  ...
  it('User 2 perform invalid fast withdraw 100 RSRUSD from L2', async () => {
    const { users, pool, rusd, helpersContract } = testEnv;
    const reserveDataBefore = await getReserveData(helpersContract, rusd.address);
    const tx = await waitForTx(
      await pool.connect(users[2].signer).mintUnbacked(rusd.address, mintAmount, users[2].address, 0)
  ...
  it('100 bridged rusd used to back unbacked', async () => {
    // Let user 3 be bridge for now
    const { users, pool, rusd, RSRUSD, helpersContract } = testEnv;
    await waitForTx(await rusd.connect(users[3].signer)['mint(uint256)'](withdrawAmount));
    await waitForTx(await rusd.connect(users[3].signer).approve(pool.address, MAX_UINT_AMOUNT));

    const reserveDataBefore = await getReserveData(helpersContract, rusd.address);

    const tx = await waitForTx(
      await pool.connect(users[3].signer).backUnbacked(rusd.address, mintAmount, feeAmount)
    );
    ...
    const expectedReserveDataAfter = calcExpectedReserveDataAfterBackUnbacked(
      await RSRUSD.scaledTotalSupply(),
      mintAmount.toString(),
      feeAmount.toString(),
  ...
  it('user 1 performs unauthorized backing', async () => {
    const { users, pool, rusd } = testEnv;
    await rusd.connect(users[1].signer)['mint(uint256)'](withdrawAmount);
    await rusd.connect(users[1].signer).approve(pool.address, MAX_UINT_AMOUNT);

    await expect(
      pool.connect(users[1].signer).backUnbacked(rusd.address, mintAmount, feeAmount)
    ).to.be.revertedWith(CALLER_NOT_BRIDGE);
  });
  ...
  it('100 bridged rusd used to back unbacked', async () => {
    // Let user 3 be bridge for now
    const { users, pool, rusd, RSRUSD, helpersContract } = testEnv;
    await rusd.connect(users[3].signer)['mint(uint256)'](withdrawAmount);
  ...
  it('User donates 100 rusd to RSRUSD holders', async () => {
    // Let user 3 be bridge for now
    const { users, pool, rusd, RSRUSD, helpersContract } = testEnv;
    await rusd.connect(users[3].signer)['mint(uint256)'](withdrawAmount);
    await rusd.connect(users[3].signer).approve(pool.address, MAX_UINT_AMOUNT);
    ...
    const tx = await waitForTx(
      await pool.connect(users[3].signer).backUnbacked(rusd.address, '0', withdrawAmount)
    );
  ...
  it('Safety module cover 100 unbacked rusd', async () => {
    // Let user 3 be bridge for now
    const { users, pool, rusd, RSRUSD, helpersContract } = testEnv;
    await rusd.connect(users[3].signer)['mint(uint256)'](withdrawAmount);
    await rusd.connect(users[3].signer).approve(pool.address, MAX_UINT_AMOUNT);
    ...
    const tx = await waitForTx(
      await pool.connect(users[3].signer).backUnbacked(rusd.address, mintAmount, '0')
    );

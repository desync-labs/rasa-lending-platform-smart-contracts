...
makeSuite('Pool: Simple FlashLoan', (testEnv: TestEnv) => {
  ...
  it('Check that reentrance borrow within flashloanSimple impacts rates', async () => {
    /**
     * 1. FlashBorrow a tiny bit of RUSD
     * 2. As the action in the middle. Borrow ALL the RUSD using eth collateral
    ...
    const dataBefore = await pool.getReserveData(rusd.address);
    ...
    const availableBefore = await rusd.balanceOf(RSRUSD.address);

    await pool
      .connect(user.signer)
      .flashLoanSimple(flashAttacker.address, rusd.address, parseUnits('1', 18), '0x10', 0);

    const dataAfter = await pool.getReserveData(rusd.address);
    const debtAfter = await debtToken.totalSupply();
    const availableAfter = await rusd.balanceOf(RSRUSD.address);

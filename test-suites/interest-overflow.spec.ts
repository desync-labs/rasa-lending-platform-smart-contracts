...
import { evmSnapshot, evmRevert, increaseTime } from '../helpers/utilities/tx';
...
makeSuite('Interest Rate and Index Overflow', (testEnv) => {
  ...
  before(async () => {
    const { pool, poolAdmin, configurator, rusd, helpersContract, addressesProvider } = testEnv;
  ...
  it('ReserveLogic `cumulateToLiquidityIndex` with liquidityIndex > type(uint128).max (revert expected)', async () => {
    ...
    await rusd.connect(user.signer)['mint(uint256)'](toBorrow.add(1));
    await rusd.connect(user.signer).approve(pool.address, MAX_UINT_AMOUNT);

    await pool.connect(user.signer).deposit(rusd.address, 1, user.address, 0);
    await rusd.connect(user.signer).transfer(RSRUSD.address, toBorrow);

...
makeSuite('PausableReserve', (testEnv: TestEnv) => {
  ...
  it('User 0 deposits 1000 RUSD. Configurator pauses pool. Transfers to user 1 reverts. Configurator unpauses the network and next transfer succeeds', async () => {
    ...
    await configurator.connect(users[1].signer).setReservePause(rusd.address, true);
    ...
    await configurator.connect(users[1].signer).setReservePause(rusd.address, false);
  ...
  it('Liquidation call', async () => {
    ...
    // Pause pool
    await configurator.connect(users[1].signer).setReservePause(eurs.address, true);
    ...
    // Unpause pool
    await configurator.connect(users[1].signer).setReservePause(eurs.address, false);
  });
  ...
  it('SwapBorrowRateMode', async () => {
    ...
    const amountToBorrow = await convertToCurrencyDecimals(eurs.address, '65');

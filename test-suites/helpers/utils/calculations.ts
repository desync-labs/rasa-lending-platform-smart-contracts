...
export const calcExpectedUserDataAfterDeposit = (
  ...
  expectedUserData.scaledRSTokenBalance = calcExpectedScaledRSTokenBalance(
    userDataBeforeAction,
    reserveDataAfterAction.liquidityIndex,
    BigNumber.from(amountDeposited),
    BigNumber.from(0)
  );
  expectedUserData.currentRSTokenBalance = calcExpectedRSTokenBalance(
    reserveDataBeforeAction,
    userDataBeforeAction,
    txTimestamp
  ).add(amountDeposited);

  if (userDataBeforeAction.currentRSTokenBalance.eq(0)) {
...
export const calcExpectedUserDataAfterWithdraw = (
  ...
  const RSTokenBalance = calcExpectedRSTokenBalance(
    reserveDataBeforeAction,
    userDataBeforeAction,
    txTimestamp
  );
  ...
  if (amountWithdrawn == MAX_UINT_AMOUNT) {
    amountWithdrawn = RSTokenBalance.toString();
  }
  ...
  expectedUserData.currentRSTokenBalance = RSTokenBalance.sub(amountWithdrawn);
  expectedUserData.principalStableDebt = userDataBeforeAction.principalStableDebt;
  expectedUserData.scaledVariableDebt = userDataBeforeAction.scaledVariableDebt;
  expectedUserData.currentStableDebt = calcExpectedStableDebtTokenBalance(
    userDataBeforeAction.principalStableDebt,
  ...
  if (userDataBeforeAction.currentRSTokenBalance.eq(0)) {
    ...
    if (expectedUserData.currentRSTokenBalance.eq(0)) {
...
export const calcExpectedReserveDataAfterBackUnbacked = (
  ...
  const totalSupply = scaledRSTokenSupply.rayMul(expectedReserveData.liquidityIndex);
...
export const calcExpectedReserveDataAfterWithdraw = (
  ...
  if (amountWithdrawn == MAX_UINT_AMOUNT) {
    amountWithdrawn = calcExpectedRSTokenBalance(
      reserveDataBeforeAction,
      userDataBeforeAction,
      txTimestamp
    ).toString();
...
export const calcExpectedUserDataAfterBorrow = (
  ...
  expectedUserData.scaledRSTokenBalance = userDataBeforeAction.scaledRSTokenBalance;
...
const calcExpectedScaledRSTokenBalance = (
  userDataBeforeAction: UserReserveData,
  index: BigNumber,
  amountAdded: BigNumber,
  amountTaken: BigNumber
) => {
  return userDataBeforeAction.scaledRSTokenBalance
    .add(amountAdded.rayDiv(index))
    .sub(amountTaken.rayDiv(index));
};

export const calcExpectedRSTokenBalance = (
  ...
  const { scaledRSTokenBalance: scaledBalanceBeforeAction } = userData;

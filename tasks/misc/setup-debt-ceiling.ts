...
task(`setup-debt-ceiling`, `Setups reserve debt ceiling from configuration`).setAction(
  async (_, hre) => {
    ...
    for (let asset in config.ReservesConfig) {
      ...
      if (debtCeiling.gt('0')) {
        ...
        console.log(
          ...
          asset,
          'at',
          (Number(config.ReservesConfig[asset].debtCeiling) / 100).toLocaleString(undefined, {

import { getRASAOracle } from './../../helpers/contract-getters';
...
task(`set-fallback-oracle`)
  ...
  .setAction(async ({ address }, { deployments, getNamedAccounts, ...hre }) => {
    ...
    const newFallbackOracleAddress = address ? getAddress(address) : ZERO_ADDRESS;

    const rasaOracle = await (
      await getRASAOracle(await getAddressFromJson(network, ORACLE_ID))
    ).connect(signer);

    await waitForTx(await rasaOracle.setFallbackOracle(newFallbackOracleAddress));

    const updatedFallbackOracle = await rasaOracle.getFallbackOracle();

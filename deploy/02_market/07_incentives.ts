...
import { EmissionManager, PoolAddressesProvider, RewardsController } from '../../types';
 ...
 * following the IncentivesController interface to be compatible with RSTokens or Debt Tokens.
 */
const func: DeployFunction = async function ({
  ...
  const isRewardsProxyPending =
    (await addressesProviderInstance.getAddress(incentivesControllerId)) === ZERO_ADDRESS;
  ...
  if (!isLive) {
    ...
    const stakedRASAAddress = isLive
      ? getParamPerNetwork(poolConfig.StkRASAProxy, network)
      : (await deployments.getOrNull(STAKE_RASA_PROXY))?.address;

    if (stakedRASAAddress) {
      await deploy(INCENTIVES_STAKED_TOKEN_STRATEGY_ID, {
        from: deployer,
        contract: 'StakedTokenTransferStrategy',
        args: [rewardsProxyAddress, incentivesEmissionManager, stakedRASAAddress],
        ...COMMON_DEPLOY_PARAMS,
      });
    } else {
      console.log(
        '[WARNING] Missing StkRASA address. Skipping StakedTokenTransferStrategy deployment.'
      );
    }
  }
...
func.id = `Incentives:${MARKET_NAME}:rasa-lending@${V3_PERIPHERY_VERSION}`;

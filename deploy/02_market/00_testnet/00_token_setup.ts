import { STAKE_RASA_PROXY, TESTNET_REWARD_TOKEN_PREFIX } from '../../../helpers/deploy-ids';
...
const func: DeployFunction = async function ({
  ...
  if (isIncentivesEnabled(poolConfig)) {
    ...
    // 3. Deployment of Stake RASA
    const COOLDOWN_SECONDS = '3600';
    const UNSTAKE_WINDOW = '1800';
    const rasaTokenArtifact = await deployments.get(`RASA${TESTNET_TOKEN_PREFIX}`);

    const stakeProxy = await deployInitializableAdminUpgradeabilityProxy(STAKE_RASA_PROXY);

    // Setup StkRASA
    await setupStkRASA(stakeProxy, [
      rasaTokenArtifact.address,
      rasaTokenArtifact.address,
      COOLDOWN_SECONDS,
      UNSTAKE_WINDOW,

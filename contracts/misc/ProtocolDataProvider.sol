...
contract ProtocolDataProvider is IPoolDataProvider {
  ...
  /// @inheritdoc IPoolDataProvider
  function getAllRSTokens() external view override returns (TokenData[] memory) {
    IPool pool = IPool(ADDRESSES_PROVIDER.getPool());
    address[] memory reserves = pool.getReservesList();
    TokenData[] memory RSTokens = new TokenData[](reserves.length);
    for (uint256 i = 0; i < reserves.length; i++) {
      DataTypes.ReserveData memory reserveData = pool.getReserveData(reserves[i]);
      RSTokens[i] = TokenData({
        symbol: IERC20Detailed(reserveData.RSTokenAddress).symbol(),
        tokenAddress: reserveData.RSTokenAddress
      });
    }
    return RSTokens;
  }
    ...
    )
  {
    ...
    return (
      ...
      IERC20Detailed(reserve.RSTokenAddress).totalSupply(),
  ...
  /// @inheritdoc IPoolDataProvider
  function getRSTokenTotalSupply(address asset) external view override returns (uint256) {
    DataTypes.ReserveData memory reserve = IPool(ADDRESSES_PROVIDER.getPool()).getReserveData(
      asset
    );
    return IERC20Detailed(reserve.RSTokenAddress).totalSupply();
  }
    ...
    )
  {
    ...
    currentRSTokenBalance = IERC20Detailed(reserve.RSTokenAddress).balanceOf(user);

...
library LiquidationLogic {
  ...
  function executeLiquidationCall(
    ...
    (
      vars.collateralRSToken,
      vars.collateralPriceSource,
      vars.debtPriceSource,
      vars.liquidationBonus
    ) = _getConfigurationData(eModeCategories, collateralReserve, params);

    vars.userCollateralBalance = vars.collateralRSToken.balanceOf(params.user);
    ...
    if (params.receiveRSToken) {
      _liquidateRSTokens(reservesData, reservesList, usersConfig, collateralReserve, params, vars);
    } else {
      _burnCollateralRSTokens(collateralReserve, params, vars);
    }
    ...
    if (vars.liquidationProtocolFeeAmount != 0) {
      ...
      uint256 scaledDownUserBalance = vars.collateralRSToken.scaledBalanceOf(params.user);
      // To avoid trying to send more RSTokens than available on balance, due to 1 wei imprecision
      if (scaledDownLiquidationProtocolFee > scaledDownUserBalance) {
        vars.liquidationProtocolFeeAmount = scaledDownUserBalance.rayMul(liquidityIndex);
      }
      vars.collateralRSToken.transferOnLiquidation(
        params.user,
        vars.collateralRSToken.RESERVE_TREASURY_ADDRESS(),
        vars.liquidationProtocolFeeAmount
      );
    }

    // todo usty : use burn/mint if non-transferable inside handleRepayment
    IERC20(params.debtAsset).safeTransferFrom(
      msg.sender,
      vars.debtReserveCache.RSTokenAddress,
      vars.actualDebtToLiquidate
    );

    IRSToken(vars.debtReserveCache.RSTokenAddress).handleRepayment(
      msg.sender,
      params.user,
      vars.actualDebtToLiquidate
    );
    ...
    emit LiquidationCall(
      ...
      vars.actualCollateralToLiquidate,
      msg.sender,
      params.receiveRSToken
    );
  }

  /**
   * @notice Burns the collateral RSTokens and transfers the underlying to the liquidator.
   * @dev   The function also updates the state and the interest rate of the collateral reserve.
   * @param collateralReserve The data of the collateral reserve
   * @param params The additional parameters needed to execute the liquidation function
   * @param vars The executeLiquidationCall() function local vars
   */
  function _burnCollateralRSTokens(
    ...
    // Burn the equivalent amount of RSToken, sending the underlying to the liquidator
    vars.collateralRSToken.burn(
      params.user,
      msg.sender,
      vars.actualCollateralToLiquidate,
  ...
  /**
   * @notice Liquidates the user RSTokens by transferring them to the liquidator.
   * @dev   The function also checks the state of the liquidator and activates the RSToken as collateral
  ...
  function _liquidateRSTokens(
    ...
    uint256 liquidatorPreviousRSTokenBalance = IERC20(vars.collateralRSToken).balanceOf(msg.sender);
    vars.collateralRSToken.transferOnLiquidation(
      params.user,
      msg.sender,
      vars.actualCollateralToLiquidate
    ...
    if (liquidatorPreviousRSTokenBalance == 0) {
      ...
      if (
        ValidationLogic.validateAutomaticUseAsCollateral(
          ...
          collateralReserve.RSTokenAddress
        )
      ) {
  ...
  function _getConfigurationData(
    ...
    DataTypes.ReserveData storage collateralReserve,
    DataTypes.ExecuteLiquidationCallParams memory params
  ) internal view returns (IRSToken, address, address, uint256) {
    IRSToken collateralRSToken = IRSToken(collateralReserve.RSTokenAddress);
    ...
    return (collateralRSToken, collateralPriceSource, debtPriceSource, liquidationBonus);
  }

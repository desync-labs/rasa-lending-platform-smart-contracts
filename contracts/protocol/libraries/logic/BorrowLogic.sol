...
library BorrowLogic {
   ...
   * RASA protocol proportionally to their collateralization power. For isolated positions, it also increases the
   * isolated debt.
  ...
  function executeBorrow(
    ...
    if (params.releaseUnderlying) {
      IRSToken(reserveCache.RSTokenAddress).transferUnderlyingTo(params.user, params.amount);
    }
  ...
  /**
   * @notice Implements the repay feature. Repaying transfers the underlying back to the RSToken and clears the
  ...
  function executeRepay(
    ...
    // Allows a user to repay with RSTokens without leaving dust from interest.
    if (params.useRSTokens && params.amount == type(uint256).max) {
      params.amount = IRSToken(reserveCache.RSTokenAddress).balanceOf(msg.sender);
    }
    ...
    reserve.updateInterestRates(
      reserveCache,
      params.asset,
      params.useRSTokens ? 0 : paybackAmount,
      0
    );
    ...
    if (params.useRSTokens) {
      IRSToken(reserveCache.RSTokenAddress).burn(
        msg.sender,
        reserveCache.RSTokenAddress,
        paybackAmount,
        reserveCache.nextLiquidityIndex
      );
    } else {
      // todo usty : move safeTransferFrom to handleRepayment
      // use burn/mint if non-transferable
      IERC20(params.asset).safeTransferFrom(msg.sender, reserveCache.RSTokenAddress, paybackAmount);
      IRSToken(reserveCache.RSTokenAddress).handleRepayment(
        msg.sender,
        params.onBehalfOf,
        paybackAmount
      );
    ...
    emit Repay(params.asset, params.onBehalfOf, msg.sender, paybackAmount, params.useRSTokens);

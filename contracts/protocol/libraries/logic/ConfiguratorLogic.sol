 ...
 * @notice Implements the functions to initialize reserves and update RSTokens and debtTokens
 */
library ConfiguratorLogic {
  ...
  event ReserveInitialized(
    ...
    address indexed RSToken,
    address stableDebtToken,
    address variableDebtToken,
    address interestRateStrategyAddress
  );
  event RSTokenUpgraded(
    address indexed asset,
  ...
  /**
   * @notice Initialize a reserve by creating and initializing RSToken, stable debt token and variable debt token
  ...
  function executeInitReserve(
    ...
    // todo usty: pass parameter isTransferable
    // consider using different implementation (bad idea, just keep note)
    address RSTokenProxyAddress = _initTokenWithProxy(
      input.RSTokenImpl,
      abi.encodeWithSelector(
        IInitializableRSToken.initialize.selector,
        pool,
        input.treasury,
  ...
  /**
   * @notice Updates the RSToken implementation and initializes it
   * @dev Emits the `RSTokenUpgraded` event
   * @param cachedPool The Pool containing the reserve with the RSToken
   * @param input The parameters needed for the initialize call
   */
  function executeUpdateRSToken(
    IPool cachedPool,
    ConfiguratorInputTypes.UpdateRSTokenInput calldata input
  ) public {
    ...
    _upgradeTokenImplementation(reserveData.RSTokenAddress, input.implementation, encodedCall);

    emit RSTokenUpgraded(input.asset, reserveData.RSTokenAddress, input.implementation);
  }

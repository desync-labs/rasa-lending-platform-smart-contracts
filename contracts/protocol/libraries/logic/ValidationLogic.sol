...
library ValidationLogic {
  ...
  function validateSupply(
    ...
    require(
      supplyCap == 0 ||
        ((IRSToken(reserveCache.RSTokenAddress).scaledTotalSupply() +
    ...
    if (params.interestRateMode == DataTypes.InterestRateMode.STABLE) {
      ...
      require(
        !params.userConfig.isUsingAsCollateral(reservesData[params.asset].id) ||
          ...
          params.amount > IERC20(params.reserveCache.RSTokenAddress).balanceOf(params.userAddress),
        Errors.COLLATERAL_SAME_AS_BORROWING_CURRENCY
      );

      vars.availableLiquidity = IERC20(params.asset).balanceOf(params.reserveCache.RSTokenAddress);
    ...
    if (currentRateMode == DataTypes.InterestRateMode.STABLE) {
      ...
      require(
        !userConfig.isUsingAsCollateral(reserve.id) ||
          ...
          stableDebt + variableDebt > IERC20(reserveCache.RSTokenAddress).balanceOf(msg.sender),
        Errors.COLLATERAL_SAME_AS_BORROWING_CURRENCY
      );
    } else {
    ...
    (uint256 liquidityRateVariableDebtOnly, , ) = IReserveInterestRateStrategy(
      reserve.interestRateStrategyAddress
        ...
        DataTypes.CalculateInterestRatesParams({
          ...
          RSToken: reserveCache.RSTokenAddress
        })
      );
   ...
   * @param from The user from which the RSTokens are being transferred
   * @param reservesCount The number of available reserves
   * @param oracle The price oracle
   * @param userEModeCategory The users active efficiency mode category
   */
  ...
  function validateDropReserve(
    ...
    require(
      IERC20(reserve.RSTokenAddress).totalSupply() == 0 && reserve.accruedToTreasury == 0,
  ...
  function validateAutomaticUseAsCollateral(
    ...
    if (reserveConfig.getDebtCeiling() != 0) {
      ...
      IPoolAddressesProvider addressesProvider = IncentivizedERC20(RSTokenAddress)
        .POOL()
        .ADDRESSES_PROVIDER();
      if (

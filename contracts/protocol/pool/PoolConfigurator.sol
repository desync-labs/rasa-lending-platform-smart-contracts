 ...
 * @dev Implements the configuration methods for the RASA protocol
 */
contract PoolConfigurator is VersionedInitializable, IPoolConfigurator {
  ...
  /// @inheritdoc IPoolConfigurator
  function updateRSToken(
    ConfiguratorInputTypes.UpdateRSTokenInput calldata input
  ) external override onlyPoolAdmin {
    ConfiguratorLogic.executeUpdateRSToken(_pool, input);
  }
  ...
  function _checkNoSuppliers(address asset) internal view {
    (, uint256 accruedToTreasury, uint256 totalRSTokens, , , , , , , , , ) = IPoolDataProvider(
      _addressesProvider.getPoolDataProvider()
    ).getReserveData(asset);

    require(totalRSTokens == 0 && accruedToTreasury == 0, Errors.RESERVE_LIQUIDITY_NOT_ZERO);
  }

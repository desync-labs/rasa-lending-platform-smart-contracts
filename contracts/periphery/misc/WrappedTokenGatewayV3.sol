...
contract WrappedTokenGatewayV3 is IWrappedTokenGatewayV3, Ownable {
  ...
  /**
   * @dev deposits WETH into the reserve, using native ETH. A corresponding amount of the overlying asset (RSTokens)
   * is minted.
   * @param onBehalfOf address of the user who will receive the RSTokens representing the deposit
   ...
   * @param amount amount of RSWETH to withdraw and receive native ETH
   * @param to address of the user who will receive native ETH
   */
  function withdrawETH(address, uint256 amount, address to) external override {
    IRSToken RSWETH = IRSToken(POOL.getReserveData(address(WETH)).RSTokenAddress);
    uint256 userBalance = RSWETH.balanceOf(msg.sender);
    ...
    RSWETH.transferFrom(msg.sender, address(this), amountToWithdraw);
  ...
  function withdrawETHWithPermit(
    ...
    RSWETH.permit(msg.sender, address(this), amount, deadline, permitV, permitR, permitS);
    RSWETH.transferFrom(msg.sender, address(this), amountToWithdraw);
    POOL.withdraw(address(WETH), amountToWithdraw, address(this));
    WETH.withdraw(amountToWithdraw);
    _safeTransferETH(to, amountToWithdraw);
